...
impl HTTPNodeClient {
	...
	fn get_connected_peer_info_impls(&self, counter: i32) -> Result<Vec<mimble_p2p::types::PeerInfoDisplayLegacy>, libwallet::Error> {
		// There is no v2 API with connected peers. Keep using v1 for that
		let addr = self.node_url();
		let url = format!("{}/v1/peers/connected", addr);

		let res = self.client
			.get::<Vec<mimble_p2p::types::PeerInfoDisplayLegacy>>(url.as_str(), self.node_api_secret());
		match res {
			Err(e) => {
				// Do retry
				if counter>0 {
	...
	fn get_outputs_from_node_impl(
		...
		let chunk_size = match env::var("mimble_OUTPUT_QUERY_SIZE") {
			Ok(s) => match s.parse::<usize>() {
				Ok(c) => c,
				Err(e) => {
					error!(
						"Unable to parse mimble_OUTPUT_QUERY_SIZE, defaulting to {}",
						chunk_default
					);
					error!("Reason: {}", e);
					chunk_default
